...
function Vehicle() {
    ...
    function getDatas() {
        axios.get(`${process.env.REACT_APP_API_URL}/vehicle`).then(function (response) {
            setData(response.data.data);
        });
    }
    const deleteData = (id) => {
        axios.delete(`${process.env.REACT_APP_API_URL}/vehicle/${id}`).then(function (response) {
            getDatas();
        });
    }
    return (
                    ...
                    <div className="col-12">
                        <div className="card">
                            <div className="card-header">
                                <h4 className="card-title">All Vehicle</h4>
                                <Link to={'/vehicle/add'} className='btn btn-primary float-right' >Add New</Link>
                            </div>
